...
export const ChapterFooterNav: React.FC<ChapterFooterNavProps> = ({ currentChapterId }) => {
  ...
  return (
    <div className="flex items-center justify-between gap-4 pt-8 pb-12 mt-12 border-t border-gold/20">
      
      {/* Previous Button */}
      {prevChapter ? (
        <button
          onClick={() => setActiveChapter(prevChapter.id)}
          className="flex items-center gap-2.5 px-4 py-3 bg-celestial-surface hover:bg-slate-800 border border-gold/30 hover:border-gold/60 rounded-xl text-slate-200 hover:text-yellow-300 transition-all group max-w-[48%]"
        >
      ...
      {nextChapter ? (
        <button
          ...
          className="flex items-center gap-2.5 px-4 py-3 bg-celestial-surface hover:bg-slate-800 border border-gold/30 hover:border-gold/60 rounded-xl text-slate-200 hover:text-yellow-300 transition-all group ml-auto max-w-[48%]"
        >
